...
from pose_detector import (is_dataset_full, is_still, is_moving,
                           DatasetNotFullError)
...
class TestIsDatasetFull(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.datasets = (pd.DataFrame(np.linspace(1,5,5).reshape(1,5)),
                         ...
                         pd.DataFrame(np.linespace(1,15,15).reshape(3,5)),
                         pd.DataFrame(np.linespace(1,20,20).reshape(4,5)))
    ...
    def test_is_dataset_full_raises_if_not_full(self):
        for i, expected_length in enumerate([2, 3, 4, 5]):
            with self.assertRaises(DatasetNotFullError):
                ...
                is_dataset_full(expected_length, dataset_length)

    def test_is_dataset_full_raises_if_dataset_length_greater_than_length(self):
...
class TestIsMoving(unittest.TestCase):
    ...
    def returns_True_if_all_data_is_above_threshold(self):
        for dataset in self.datasets[1:]:
            self.assertTrue(is_still(0, dataset))
