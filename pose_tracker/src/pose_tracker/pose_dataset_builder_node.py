...
def only_in_states(states):
    ...
    :states (list): states in which the decorated method can be called
    ...
    Example:
        ...
        'f1 called!'
    """
    # Convert an object to an iterable (unless already it is one)
...
class PoseDatasetBuilder(object):
    ...
    def __init__(self, **kwargs):
        ...
        rospy.Subscriber("skeletons", kin.NiteSkeletonList,
    ...
    def load_parameters(self):
        ...
        try:
            ...
            rospy.signal_shutdown("node " + rospy.get_name() +
                                  " shot down because params were not found")
    ...
    def command_callback(self, command):
        """Process the command and set the curr state.
    ...
    def _skeleton_cb_preconditions(self, skeletons):
        if self.curr_state != STATE_PROCESSING:
            raise PreconditionError("Current state is not {}"
                                    .format(STATE_PROCESSING))
    ...
    def handle_dsinfo_srv(self):
        """
        Service callback that returns info of the dataset being used.
    ...
    def create_dataset(self):
        """Create dataset file and fills metadata."""
    ...
    def state_idle(self):
        """Perform the operations of the sate "idle"."""
        # loginfo('State: Idle')
        pass
    ...
    def _state_processing_precons(self):
        ...
        if not self.skeleton_queue:
            raise PreconditionError(
                'Processing state, but Skeletons Queue is empty')
    ...
    def _change_state_preconditions(self, new_state):
        ...
        if new_state not in self.transitions.get(self.curr_state):
            raise PreconditionError(
                "Invalid state transition. State not changed")
    ...
    def change_state(self, new_state):
        ...
        @param new_state: New state to set. Should be included in I{ALL_STATES}
